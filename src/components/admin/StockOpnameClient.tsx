  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold" style={{ color: "#EADBC8", fontFamily: "Playfair Display, serif" }}>Stock Opname</h1>
      ...
      {showNew && (
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="rounded-2xl p-5" style={{ background: "#181818", border: "1px solid rgba(192,139,92,0.2)" }}>
          <h2 className="font-semibold mb-4" style={{ color: "#EADBC8" }}>Input Stok Fisik</h2>
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {ingredients.map((ing) => (
              <div key={ing.id} className="flex items-center gap-4 py-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                <span className="flex-1 text-sm" style={{ color: "#EADBC8" }}>{ing.name}</span>
                <span className="text-xs w-20 text-right" style={{ color: "rgba(216,198,181,0.5)" }}>Sistem: {parseFloat(ing.stock).toLocaleString("id-ID")} {ing.unit}</span>
                ...
                <span className="text-xs w-8" style={{ color: "rgba(216,198,181,0.5)" }}>{ing.unit}</span>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: "#181818", border: "1px solid rgba(255,255,255,0.05)" }}>
        {opnames.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16">
            <ClipboardList size={32} color="rgba(216,198,181,0.2)" />
            <p className="mt-3 text-sm" style={{ color: "rgba(216,198,181,0.4)" }}>Belum ada stock opname</p>
          </div>
        ) : (
            ...
            <tbody>
              {opnames.map((o) => (
                <tr key={o.id}>
                  ...
                  <td style={{ color: "rgba(216,198,181,0.7)" }}>{o.itemCount} bahan</td>
                  <td style={{ color: "rgba(216,198,181,0.7)" }}>{o.performedBy || "-"}</td>
